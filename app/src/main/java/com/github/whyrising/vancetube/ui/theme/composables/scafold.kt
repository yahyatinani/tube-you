 ...
 *
 * @param isFabDocked whether the FAB (if present) is docked to the bottom bar
 * or not.
 * @param fabPosition [FabPosition] for the FAB (if present)
 * @param topBar the content to place at the top of the [Scaffold], typically a
 * [TopAppBar].
 ...
 * @param bottomBar the content to place at the bottom of the [Scaffold], on
 * top of the [content], typically a [BottomAppBar].
 */
@Composable
@UiComposable
private fun ScaffoldLayout(
  ...
  SubcomposeLayout { constraints ->
    ...
    layout(layoutWidth, layoutHeight) {
      ...
      val snackbarHeight =
        snackbarPlaceables.fastMaxBy { it.height }?.height ?: 0
      ...
      val bottomBarHeight =
        bottomBarPlaceables.fastMaxBy { it.height }?.height ?: 0
      val fabOffsetFromBottom = fabPlacement?.let {
        if (bottomBarHeight == 0) {
          ...
          if (isFabDocked) {
            ...
            // Total height is the bottom bar height + the FAB height + the
            // padding between the FAB and bottom bar
      ...
      // Placing to control drawing order to match default elevation of each
      // placeable
      ...
      // Explicitly not using placeRelative here as `leftOffset` already
      // accounts for RTL
 ...
 * @property left the FAB's offset from the left edge of the bottom bar,
 * already adjusted for RTL support
...
/**
 * CompositionLocal containing a [FabPlacement] that is read by [BottomAppBar]
 * to calculate notch location.
 */
internal val LocalFabPlacement =
  staticCompositionLocalOf<FabPlacement?> { null }
